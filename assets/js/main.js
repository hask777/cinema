jQuery(document).ready(function ($) {
	...
	$('.section__btn').on('click', function(e){
		...
		var api_url = 'https://videocdn.tv/api/movies'
		var key = 'lTf8tBnZLmO0nHTyRaSlvGI5UH1ddZ2f&limit=90' // not real

		$.ajax({
		    url: api_url + "?api_token=" + key,
		    ...
		    success: function(result){
                ...
                <div class="row uza-portfolio">
                    ${result.data.map(item =>
                        `<div class="col-12 col-md-2 single-portfolio-item market-analytics" style="color:#fff">
                            ${item.ru_title}
							${item.iframe}
                        </div>`
                    ).join('')}
